...
//------------------------------------------------------------------------------
void
my_string::trim_quotes(){ //rm all ' and " at beginning and end
	char* line=new char[_nb_char+1];
   int new_len=0;
   int i=0;
   while(_data[i]=='"' || _data[i]=='\'') ++i;
...
//------------------------------------------------------------------------------
void
my_string::trim_quotes_and_blanks(){ //rm all ', ", ' ' and '\t' at beginning and end
	char* line=new char[_nb_char+1];
   int new_len=0;
   int i=0;
   while(_data[i]=='"' || _data[i]=='\'' || _data[i]==' ' || _data[i]=='\t') ++i;
   int j=_nb_char-1;
   while(_data[j]=='"' || _data[j]=='\'' || _data[i]==' ' || _data[i]=='\t') --j;
   for(;i<=j;i++){
	   line[new_len]=_data[i];
       new_len++;
   }
...
//------------------------------------------------------------------------------
/** returns the dobule and if the string is not an double it returns NPOS */
double
my_string::toDouble(){
   char input[80], *endptr;
   double d=strtod(_data, &endptr);  // endptr is null if everything is fine
   //if(endptr) return NPOS;
   return d;
}
